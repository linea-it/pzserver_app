...
from core.models import GroupMembership, GroupMetadata, Release, Product, ProductType, Profile
...
class TestSAML2GroupManagementBase(TransactionTestCase):
    ...
    def tearDown(self):
        ...
        ProductType.objects.exclude(id__in=[1, 2, 3, 4, 5, 6, 7]).delete()  # Keep from fixtures
        Product.objects.all().delete()
        Profile.objects.exclude(user=self.test_user).delete()
        User.objects.exclude(username="testuser").delete()

    def create_saml_attributes(self, uid="testuser", status="Active", groups=None, 
                             display_name="Test User", email="testuser@example.com"):
        """
        ...
        return {
            ...
            "member": groups,
            "displayName": [display_name] if isinstance(display_name, str) else display_name,
...
class TestLineaGroupDeletion(TestSAML2GroupManagementBase):
    ...
    def test_deactivate_group_metadata_when_no_members(self):
        """Testa desativação de metadata quando grupo não tem membros ativos."""
        # Remove usuário de ambos os grupos
        current_groups = []
        
        with self.assertLogs('saml', level='INFO') as cm:
            self.backend._deactivate_missing_linea_groups(self.test_user, current_groups)
        
        # Verifica se as metadatas foram desativadas
        self.linea_group1.metadata.refresh_from_db()
        self.linea_group2.metadata.refresh_from_db()
        
        self.assertFalse(self.linea_group1.metadata.is_active)
        self.assertFalse(self.linea_group2.metadata.is_active)
        
        # Verifica logs de desativação
        self.assertTrue(any("deactivated" in message for message in cm.output))
    ...
    def test_deactivate_preserve_group_with_other_active_members(self):
        ...
        # Metadata do Group1 deve permanecer ativa devido ao other_user
        self.linea_group1.metadata.refresh_from_db()
        self.assertTrue(self.linea_group1.metadata.is_active)
        
        # Metadata do Group2 deve ser desativada
        self.linea_group2.metadata.refresh_from_db()
        self.assertFalse(self.linea_group2.metadata.is_active)

    def test_deactivate_handles_group_without_metadata(self):
        ...
        self.assertTrue(any("doesn't have correct LIneA metadata" in message for message in cm.output))
...
class TestAccessControlService(TestSAML2GroupManagementBase):
    ...
    def test_get_user_groups_active_only(self):
        """Testa obtenção de grupos ativos do usuário."""
        # Adicionar usuário aos grupos
        self.test_user.groups.add(self.access_group1, self.access_group2, self.inactive_group)
        
        active_groups = AccessControlService.get_user_groups(self.test_user, include_inactive=False)
        group_names = [g.name for g in active_groups]
        
        self.assertIn("AccessGroup1", group_names)
        self.assertIn("AccessGroup2", group_names)
        self.assertNotIn("InactiveGroup", group_names)

    def test_get_user_groups_include_inactive(self):
        """Testa obtenção de grupos incluindo inativos."""
        self.test_user.groups.add(self.access_group1, self.access_group2, self.inactive_group)
        
        all_groups = AccessControlService.get_user_groups(self.test_user, include_inactive=True)
    ...
    def test_filter_accessible_releases_admin_user(self, mock_log):
        ...
        accessible_releases = AccessControlService.filter_accessible_releases(self.test_user)
    ...
    def test_add_user_to_group_success(self):
        ...
        result = AccessControlService.add_user_to_group(self.test_user, self.access_group1)
    ...
    def test_remove_user_from_linea_group_without_force(self):
        ...
        result = AccessControlService.remove_user_from_group(self.test_user, self.access_group1)
    ...
    def test_remove_user_from_linea_group_with_force(self):
        ...
        self.assertTrue(result)
        self.assertFalse(self.test_user.groups.filter(id=self.access_group1.id).exists())
    ...
    def test_remove_user_from_local_group(self):
        ...
        result = AccessControlService.remove_user_from_group(self.test_user, self.local_group)
    ...
    def test_get_group_members_inactive_group(self):
        """Testa obtenção de membros de grupo inativo."""
        self.inactive_group.user_set.add(self.test_user)
        
        # Com active_only=True (padrão)
        members = AccessControlService.get_group_members(self.inactive_group, active_only=True)
        self.assertEqual(members.count(), 0)
        
        # Com active_only=False
        members = AccessControlService.get_group_members(self.inactive_group, active_only=False)
        self.assertIn(self.test_user, members)
...
class TestSAML2IntegrationScenarios(TestSAML2GroupManagementBase):
    ...
    def test_complete_user_login_flow(self):
        ...
        with patch('djangosaml2.backends.Saml2Backend.authenticate') as mock_super_auth:
            ...
            with patch.object(self.backend, '_update_user', return_value=None) as mock_update:
    ...
    def test_user_group_changes_between_logins(self):
        ...
        self.assertTrue(
            self.test_user.groups.filter(name="LinEA_Group4").exists()
        )
        
        # Verificar se Group2 foi removido do usuário
        self.assertFalse(
            self.test_user.groups.filter(name="LinEA_Group2").exists()
        )
        
        # Verificar se membership do Group2 foi desativada
        group2 = Group.objects.get(name="LinEA_Group2")
        membership2 = GroupMembership.objects.get(user=self.test_user, group=group2)
        self.assertFalse(membership2.is_active)
    ...
    def test_multiple_users_same_groups(self):
        ...
        self.backend._deactivate_missing_linea_groups(user2, current_groups)
        ...
        for group_name in shared_groups:
            ...
            # Grupo deve permanecer ativo pois ainda tem membros
            self.assertTrue(group.metadata.is_active)
    ...
    def test_error_handling_during_sync(self):
        ...
        with patch('core.models.GroupMetadata.objects.get_or_create') as mock_create:
            ...
            self.assertTrue(any("Error synchronizing" in message for message in cm.output))
...
class TestSAML2DataCleanup(TestSAML2GroupManagementBase):
    ...
    def test_cleanup_memberships_and_metadata(self):
        ...
        self.assertTrue(GroupMetadata.objects.filter(
            source=GroupMetadata.GroupSource.LINEA
        ).exists())
...
class TestSAML2Performance(TestSAML2GroupManagementBase):
    ...
    def test_sync_many_groups_performance(self):
        ...
        self.assertLess(execution_time, 5.0, f"Sync took {execution_time:.2f}s, too slow!")
