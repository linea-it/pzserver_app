...
@shared_task(bind=True)
def build_product_main_file_archive(self, archive_id):
    """Builds/caches a product main-file download artifact."""
    started_at = time.monotonic()
    archive = ProductDownloadArchive.objects.select_related(
        "product",
        "product__product_type",
    ...
    LOGGER.info(
        "build_product_main_file_archive started task_id=%s archive_id=%s product_id=%s",
        task_id,
        archive.pk,
        archive.product_id,
    )
    ...
    try:
        ...
        elapsed = time.monotonic() - started_at
        LOGGER.exception(
            "build_product_main_file_archive failed task_id=%s archive_id=%s product_id=%s elapsed_seconds=%.3f",
            task_id,
            archive_id,
            archive.product_id,
            elapsed,
    ...
    elapsed = time.monotonic() - started_at
    LOGGER.info(
        "build_product_main_file_archive finished task_id=%s archive_id=%s product_id=%s elapsed_seconds=%.3f archive_path=%s size=%s",
        task_id,
        archive.pk,
        archive.product_id,
        elapsed,
